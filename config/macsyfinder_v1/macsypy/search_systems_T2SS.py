        ...
        if not self.putative_system or force:
            ...
            if len(genes) == 1 and self.hits[0].gene.loner == False:
                ...
                if len(systems_compat.keys()) == 1:
                    ...
                    #print "hit : ", [h.id for h in self.hits]
                    #print "systems_compat : ", systems_compat
                    #print max(systems_compat.values()), len(self.hits)
                    #print "length cluster_compatible_systems : ", cluster_compatible_systems, len(cluster_compatible_systems)
                    #print "self._putative_system in cluster_compatible_systems : ", self._putative_system in cluster_compatible_systems
                    #print "[self._putative_system] + cluster_compatible_systems :", [self._putative_system] + cluster_compatible_systems
                    
                    if systems_compat.values() != [] and max(systems_compat.values()) == len(self.hits) and len(cluster_compatible_systems) >= 1 and self._putative_system in cluster_compatible_systems and max(systems_compat.values()) == systems_compat[self._putative_system]:
                        self._state = "clear"
